...
vim.keymap.set('n', '<space>gg', function() require("telescope").extensions.live_grep_args.live_grep_args({ cwd = string.gsub(vim.fn.input("Grep files Root > ", vim.fn.expand("%:h"), "dir"), "oil://", "") }) end)
...
vim.keymap.set('n', '<space>ff', function() require("telescope").extensions.menufacture.find_files({ cwd = string.gsub(vim.fn.input("Find files Root > ", vim.fn.expand("%:h"), "dir"), "oil://", "") }) end)
